...
bibliography: ../Resources/council-of-science-editors-brackets.csl
---
...
#| fig-cap: Interactive Folium world map with overlay over Asia showing the capital cities of interest, with a tile by @national_geographic_esrinatgeoworldmap_2023 and administrative boundaries from @opendatasoft_world_2022.

width_percentage = "100%"
IFrame(src='resources/Outputs/Plots/map_of_airport_locations.html', width=width_percentage, height=500)
```


We collected these countries' flight prices, duration, and stopover data using the SkyScanner API [@skyscanner_api_2023]. We made several simplifying assumptions in the data collection. We only obtained data for the single, arbitrary date of 22 May 2023. Also, the data does not  include the layover time between segments, only the in-flight time ^[For clarification, we cannot take the difference between the first segment departure and last segment arrival to calculate the total itinerary time because they flight data is provided in local time, which would not account for time zone changes.]. Lastly, the data collected only corresponds to economy cabin class only. Relaxation of these assumptions can be explored in future work.

Given the 42 airports, we completed three transition matrices with 1,722 non-zero entries, each matrix corresponding to either flight price, duration, or stopover information. The entries correspond to each of the $42 \times 42$ transitions minus the diagonal, which would correspond to traveling from an airport to itself ($42 * 42 - 42 = 1,722$). The search resulted in 38,236 segments, narrowed to 15,396 itineraries, and narrowed to the 1,722 'best' flights. To choose the ‘best’ flight, we used the heuristic of valuing each flight hour at $20 and every stop over at $50, resulting in $\rm best \, price =  fare + flight \,time * 20 + stops * 50$. @fig-airport-links shows color-coded prices between airports.


```{python}
#| echo: false
#| label: fig-airport-links
#| fig-cap: Interactive Folium world map showing the links between airports with color-coded prices.

width_percentage = "100%"
IFrame(src='resources/Outputs/Plots/map_of_airport_links.html', width=width_percentage, height=500)
```
...
![Flight Score Results](resources/Outputs/Plots/flight_score.png){#fig-score-results fig-align="center" width=85%}

![Fare Expense Results](resources/Outputs/Plots/fares_in_usd.png){#fig-expense-results fig-align="center" width=85%}

![Flight Time Results](resources/Outputs/Plots/flight_time.png){#fig-time-results fig-align="center" width=85%}

![Flight Stops Results](resources/Outputs/Plots/flight_stops.png){#fig-stops-results fig-align="center" width=85%}
...
#| fig-cap: Shows the first episode's links

width_percentage = "100%"
IFrame(src='resources/Outputs/Plots/first_episode_links.html', width=width_percentage, height=500)
```

```{python}
#| echo: false
#| label: fig-last-episode-links
#| fig-cap: Shows the last episode's links

width_percentage = "100%"
IFrame(src='resources/Outputs/Plots/last_episode_links.html', width=width_percentage, height=500)
```
